<template>
  <a-modal
    :maskClosable="false"
    :title="$t('table.action.add-new-menu')"
    :width="600"
    :visible="value"
    @ok="handleSubmit"
    @cancel="closeModal"
  ...
  >
    <a-form :form="form" :label-col="{ span: 7 }" :wrapper-col="{ span: 16 }">
      <a-form-item :label="$t('form.menu-title')" labelAlign="right">
        <a-input
          :maxLength="100"
          :placeholder="$t('form.menu-title')"
          v-decorator="[
            'menuTitle',
            {
              rules: [{ required: true, message: $t('error.menu-title.required') }]
            }
          ]"
        />
      </a-form-item>
      <a-form-item :label="$t('form.component-name')" labelAlign="right">
        <a-input
          :maxLength="100"
      ...
      <a-form-item :label="$t('form.page-url')" labelAlign="right">
        <a-input
          :maxLength="100"
      ...
      <a-form-item :label="$t('form.show-menu')" labelAlign="right">
        <a-select
          v-decorator="[
            'showMenu',
            {
      ...
      <a-form-item :label="$t('form.cache-component')" labelAlign="right">
        <a-select
          v-decorator="[
            'cacheComponent',
            {
      ...
      <a-form-item :label="$t('form.level')" labelAlign="right">
        <a-input
          :maxLength="100"
...
<script>
const fields = ['menuTitle', 'componentName', 'pageUrl', 'showMenu', 'cacheComponent', 'level', 'sort']
...
export default {
  ...
  computed: {
    levelValue() {
      ...
      return dict[this.selectedItemType] || 0
    },
    showMenuOptions() {
      return [
      ...
      this.form.validateFields(async (errors, values) => {
        if (!errors) {
          ...
          try {
            ...
            await this.$store.dispatch('menu-resources/addMenu', data)
            this.closeModal()
            this.$emit('refresh')
            this.showNotification(this.$t('notification.menu.added'))
          } catch (e) {
            console.log(e)
